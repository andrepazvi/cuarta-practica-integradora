...
class UsersServices {
  ...
  resetPassByEmail = async (email, res, req) => {
    ...
    `;
      ...
      const attachments = [
        {
          filename: 'opticacrisan.png',
          path: path.join(__dirname, '../../../uploads/mail/opticacrisan.png'),
        },
      ];

      const emailPayload = {
        from: 'optica.crisan@gmail.com',
        to: user.email,
        subject: 'Optica Crisan - Reestablecimiento de contraseña',
        html: emailContent,
        attachments,
      };
